...
def f(s,cursor): # find bound
	tmp = ""
	for x in s:
		tmp += "'"+x+"',"
	tmp = tmp[:len(tmp)-1]
	cursor.execute("select min(經度座標) from snapshot where id in ("+tmp+");")
	for r in cursor:
        	min_gin = r[0]
	        break
	cursor.execute("select max(經度座標) from snapshot where id in ("+tmp+");")
	for r in cursor:
        	max_gin = r[0]
	        break
	cursor.execute("select min(緯度座標) from snapshot where id in ("+tmp+");")
	for r in cursor:
        	min_wei = r[0]
	        break
	cursor.execute("select max(緯度座標) from snapshot where id in ("+tmp+");")
	for r in cursor:
        	max_wei = r[0]
	        break
	return min_gin,max_gin,min_wei,max_wei
	...
	if NewestCase != CheckPointNew : # detect new point, rule match and update
		...
		for x in Llist[len(Llist)-1]:
			if x.issubset(NeighberOfNewNode):
				#print CheckPointNew,
				print CheckPointNew+"\t"+str(f(x.union(set([CheckPointNew])),cursor))
		print "\n"
